...
**Status:** resolved via grilling session (2026-07-19), implemented (2026-07-20), corrected (2026-07-20). First implementation landed in `TranscriptionHistoryView.swift` / `TranscriptionDetailView.swift` — this turned out to be the WRONG view: it's only reachable via the legacy `MenuBarExtra` popover → "History" button → floating `NSWindow` (`HistoryWindowController`), not the app's actual primary History screen. The real primary History surface (reached from the main window's left-nav sidebar → "History") is `InlineHistoryView.swift`, a completely different flat-list implementation. The integration below is now duplicated in both views — `InlineHistoryView.swift` (primary, what the user actually sees) and `TranscriptionHistoryView.swift`/`TranscriptionDetailView.swift` (secondary, still reachable via the menu bar extra, left as-is rather than reverted since it's functionally correct and shares the same `GoldenEvalSetService`/data model, so no divergence risk).
