...
Last verified: VoiceInk **2.20** (`origin/main` `d7b528a`), Direct Typing rebased from commit `d07ff58` (previous fork tip `1b76514`). Build used Xcode 27 beta (`Xcode-beta.app`) plus Metal toolchain `27A5218h`.
...
As of 2.20, `startPasteAtCursor` returns `Task<PasteOutcome, Never>`. `typeTextDirectly` must return `PasteOutcome`, not a bare `PasteResult`. When Auto Learn is enabled, call `AutoLearnService.shared.pasteDidFinish` after the keys are posted (same as `performPasteSession`) and store that generation on the outcome so Finish and Send can cancel it. Direct typing still must not touch the clipboard.
...
5. `typeTextDirectly` must:
   ...
   - Special-case `\n`/`\r` → Return, `\t` → Tab.
   - Use Shift+Return for embedded newlines **only** when the configured Finish and Send key is Return. On 2.20+ that is `FinishAndSendSettings.selectedKey == .enter`. The 2.13 per-mode `ModeManager.shared.currentActiveConfiguration?.autoSendKey` property is gone.
   - Fall back to Unicode injection for unmapped characters (emoji, dead keys).
   - Sleep ~5ms between keys.

Never call `PowerModeManager` (renamed to `ModeManager` before 2.13) and never read `autoSendKey` off `ModeConfig` (removed in 2.20). Confirm:

```bash
rg -n 'PowerModeManager|autoSendKey|FinishAndSendSettings' \
  VoiceInk/Infrastructure/SystemIntegration/Paste/CursorPaster.swift
```

`CursorPaster` should mention `FinishAndSendSettings` and must not mention `PowerModeManager` or `autoSendKey`.
...
- Add key `"Direct Typing"` (`de`: Direktes Tippen, `fr`: Saisie directe, `zh-Hans`: 直接键入). Insert after the `"AppleScript"` entry if that is still the neighbor.
- Replace the paste-method InfoTip key with the longer string; keep existing translations and extend them. 2.20 added `fr`. Extend `de`, `fr`, and `zh-Hans`. If newer locales appear, add them too.
...
| `cannot find 'AutoSendKey'` / `autoSendKey` | 2.20 moved send key to Finish and Send | `FinishAndSendSettings.selectedKey == .enter` |
| `PasteResult` vs `PasteOutcome` | 2.20 `startPasteAtCursor` returns `PasteOutcome` | Return `PasteOutcome`; include the Auto Learn generation when enabled |
