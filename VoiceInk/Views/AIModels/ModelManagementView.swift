...
struct ModelManagementView: View {
    ...
    private var defaultModelSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(modelManager.currentTranscriptionModel?.displayName ?? "No model selected")
                .font(.title2)
                .fontWeight(.bold)
        }
        .padding()
    ...
    private var languageSelectionSection: some View {
        LanguageSelectionView(modelManager: modelManager, displayMode: .full, whisperPrompt: whisperPrompt)
    }
                ...
                VStack(spacing: 12) {
                    ForEach(filteredModels, id: \.id) { model in
                        ...
                        ModelCardRowView(
                            ...
                            isDownloaded: modelManager.availableModels.contains { $0.name == model.name },
                            isCurrent: modelManager.currentTranscriptionModel?.name == model.name,
                            downloadProgress: modelManager.downloadProgress,
                            modelURL: modelManager.availableModels.first { $0.name == model.name }?.url,
                            isWarming: isWarming,
                            deleteAction: {
                                ...
                                } else if let downloadedModel = modelManager.availableModels.first(where: { $0.name == model.name }) {
                                    alertTitle = "Delete Model"
                                    alertMessage = "Are you sure you want to delete the model '\(downloadedModel.name)'?"
                                    deleteActionClosure = {
                                        Task {
                                            await modelManager.deleteModel(downloadedModel)
                                        }
                                    }
                                    isShowingDeleteAlert = true
                                }
                            ...
                            setDefaultAction: {
                                Task {
                                    await modelManager.setDefaultTranscriptionModel(model)
                                }
                            },
                            downloadAction: {
                                if let localModel = model as? LocalModel {
                                    Task { await modelManager.downloadModel(localModel) }
                                }
                            },
    ...
    private var filteredModels: [any TranscriptionModel] {
        ...
        case .recommended:
            return modelManager.allAvailableModels.filter {
        ...
        case .local:
            return modelManager.allAvailableModels.filter { $0.provider == .local || $0.provider == .nativeApple || $0.provider == .parakeet }
        case .cloud:
            let cloudProviders: [ModelProvider] = [.groq, .elevenLabs, .deepgram, .mistral, .gemini, .soniox]
            return modelManager.allAvailableModels.filter { cloudProviders.contains($0.provider) }
        case .custom:
            return modelManager.allAvailableModels.filter { $0.provider == .custom }
        }
    }
    ...
    private func presentImportPanel() {
        ...
        if panel.runModal() == .OK, let url = panel.url {
            Task { @MainActor in
                await modelManager.importLocalModel(from: url)
            }
        }
    }
}
