...
struct OnboardingModelDownloadView: View {
    ...
    private func checkModelStatus() {
        if engine.availableModels.contains(where: { $0.name == turboModel.name }) {
            isModelSet = engine.currentTranscriptionModel?.name == turboModel.name
        }
    }
    ...
    private func handleAction() {
        if isModelSet {
        ...
        } else if engine.availableModels.contains(where: { $0.name == turboModel.name }) {
            if let modelToSet = engine.allAvailableModels.first(where: { $0.name == turboModel.name }) {
                Task {
                    await engine.setDefaultTranscriptionModel(modelToSet)
                    withAnimation {
                        isModelSet = true
                    }
                }
            ...
            Task {
                await engine.downloadModel(turboModel)
                if let modelToSet = engine.allAvailableModels.first(where: { $0.name == turboModel.name }) {
                    await engine.setDefaultTranscriptionModel(modelToSet)
                    withAnimation {
                        isModelSet = true
