...
struct ConfigurationView: View {
    ...
    private func languageSelectionDisabled() -> Bool {
        guard let selectedModelName = effectiveModelName,
              let model = modelManager.allAvailableModels.first(where: { $0.name == selectedModelName })
        else {
            return false
        }
    ...
    private var effectiveModelName: String? {
        ...
        return modelManager.currentTranscriptionModel?.name
    }
    ...
    var body: some View {
        Form {
            ...
            Section("Transcription") {
                if modelManager.usableModels.isEmpty {
                    ...
                    let modelBinding = Binding<String?>(
                        get: { selectedTranscriptionModelName ?? modelManager.currentTranscriptionModel?.name },
                        set: { selectedTranscriptionModelName = $0 }
                    )

                    Picker("Model", selection: modelBinding) {
                        ForEach(modelManager.usableModels, id: \.name) { model in
                    ...
                    .onChange(of: selectedTranscriptionModelName) { _, newModelName in
                        ...
                        if let modelName = newModelName ?? modelManager.currentTranscriptionModel?.name,
                           let model = modelManager.allAvailableModels.first(where: { $0.name == modelName }),
                ...
                if languageSelectionDisabled() {
                    ...
                    .onAppear {
                        selectedLanguage = "auto"
                          ...
                          let modelInfo = modelManager.allAvailableModels.first(where: { $0.name == selectedModel }),
