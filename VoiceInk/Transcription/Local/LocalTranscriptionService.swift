...
class LocalTranscriptionService: TranscriptionService {
    ...
    init(modelsDirectory: URL, modelManager: ModelManager? = nil) {
    ...
    func transcribe(audioURL: URL, model: any TranscriptionModel) async throws -> String {
        ...
        if let modelManager = modelManager,
           await modelManager.isModelLoaded,
           let loadedContext = await modelManager.whisperContext,
            let currentModel = await modelManager.currentTranscriptionModel,
            ...
            let resolvedURL: URL? = await modelManager?.availableModels.first(where: { $0.name == model.name })?.url
        ...
        if await modelManager?.whisperContext !== whisperContext {
