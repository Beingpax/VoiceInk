...
extension APIKeyManager {
    ...
    /// Adds a new key entry. Returns the created entry.
    @discardableResult
    ...
    /// Updates label / disabled / failure fields on an existing entry.
    @discardableResult
    func updateAPIKey(
        id: UUID,
        label: String? = nil,
    ...
    /// Advances the active index to the next enabled key.
    @discardableResult
    ...
    /// Marks a specific key entry as having failed.
    @discardableResult
    ...
    /// Sets the active index to a specific key id.
    @discardableResult
