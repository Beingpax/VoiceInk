...
class CursorPaster {
    ...
    @MainActor
    private static func typeTextDirectly(_ text: String) async -> PasteOutcome {
        guard AXIsProcessTrusted() else {
        ...
        for character in text {
            if let keyStroke = keyStroke(for: character, in: keyStrokeMap, sendKey: sendKey) {
        ...
        return PasteOutcome(result: .commandPosted, autoLearnGeneration: autoLearnGeneration)
    }
    ...
    private static func keyStroke(
        ...
        case "\n", "\r":
            ...
            // terminals and form fields. If Return is the configured Finish and
            // Send key, use Shift+Return for embedded line breaks so the final
            // send remains the only intentional submission keystroke.
            return KeyStroke(keyCode: CGKeyCode(kVK_Return), shift: sendKey == .enter)
        case "\t":
