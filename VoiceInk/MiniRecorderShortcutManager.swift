...
class MiniRecorderShortcutManager: ObservableObject {
    ...
    @objc private func settingsDidChange() {
        Task {
            if await whisperState.isRecorderVisible {
    ...
    private func setupVisibilityObserver() {
        visibilityTask = Task { @MainActor in
            for await isVisible in whisperState.$isRecorderVisible.values {
                if isVisible {
    ...
    private func setupEscapeHandlerOnce() {
        ...
        KeyboardShortcuts.onKeyDown(for: .escapeRecorder) { [weak self] in
            Task { @MainActor in
                guard let self = self,
                      await self.whisperState.isRecorderVisible else { return }
    ...
    private func setupCancelHandlerOnce() {
        ...
        KeyboardShortcuts.onKeyDown(for: .cancelRecorder) { [weak self] in
            Task { @MainActor in
                guard let self = self,
                      await self.whisperState.isRecorderVisible,
