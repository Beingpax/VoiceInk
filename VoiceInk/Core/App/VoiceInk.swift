...
struct VoiceInkApp: App {
    ...
    init() {
        ...
        let modelManager = ModelManager(modelsDirectory: modelsDirectory)
        _modelManager = StateObject(wrappedValue: modelManager)

        let whisperState = WhisperState(modelManager: modelManager, modelContext: container.mainContext, enhancementService: enhancementService)
        _whisperState = StateObject(wrappedValue: whisperState)
        
        let hotkeyManager = HotkeyManager(whisperState: whisperState, modelManager: modelManager, modelContext: container.mainContext, enhancementService: enhancementService)
        _hotkeyManager = StateObject(wrappedValue: hotkeyManager)

        let menuBarManager = MenuBarManager()
        _menuBarManager = StateObject(wrappedValue: menuBarManager)
        menuBarManager.configure(modelContainer: container, enhancementService: enhancementService)

        let activeWindowService = ActiveWindowService.shared
        activeWindowService.configure(with: enhancementService)
        _activeWindowService = StateObject(wrappedValue: activeWindowService)

        let prewarmService = ModelPrewarmService(modelManager: modelManager, modelContext: container.mainContext)
